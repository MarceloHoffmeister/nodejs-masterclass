...
handlers._checks.post = function (data, callback) {
    // Validate inputs
    const rawProtocol = data.payload.protocol
    const protocol = typeof(rawProtocol) === 'string' && ['http', 'https'].indexOf(rawProtocol) > -1 ? rawProtocol : false

    const rawUrl = data.payload.url
    const url = typeof(rawUrl) === 'string' && rawUrl.trim().length > 0 ? rawUrl.trim() : false

    const rawMethod = data.payload.method
    const method = typeof(rawMethod) === 'string' && ['post', 'get', 'put', 'delete'].indexOf(rawMethod) > -1 ? rawMethod : false

    const rawSuccessCodes = data.payload.successCodes
    const successCodes = typeof(rawSuccessCodes) === 'object' && rawSuccessCodes instanceof Array && rawSuccessCodes.length > 0 ? rawSuccessCodes : false

    const rawTimeoutSeconds = data.payload.timeoutSeconds
    const timeoutSeconds = typeof(rawTimeoutSeconds) === 'number' && rawTimeoutSeconds % 1 === 0 && rawTimeoutSeconds >= 1 && rawTimeoutSeconds <= 5 ? rawTimeoutSeconds : false

    if (protocol && url && method && successCodes && timeoutSeconds) {
        // Get the token from the headers
        const token = typeof(data.headers.token) === 'string' ? data.headers.token : false

        // Look the use by reading the token
            ...
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        // Verify the user has the less than the number of max-checks-per-user
    ...
    if (id) {
        ...
        _data.read('checks', id, function (err, checkData) {
            if (!err && checkData) {
                ...
                // Verify tha the given token is valid and belongs to the use who created the check
                                ...
                                _data.read('users', checkData.userPhone, function (err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, {'Error': 'Could not find the check on the users object, so could not remote it'})
                                        }
                                    } else {
